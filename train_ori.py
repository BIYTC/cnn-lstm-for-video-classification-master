...
config_args.experiment_dir, config_args.summary_dir, config_args.checkpoint_dir = create_experiment_dirs(
...
model = LSTMCNN(config_args)
if config_args.cuda:
    model.cuda(config_args.cuda_select)
...
for epoch in range(config_args.num_epochs):
    ...
    if val_loss < val_loss_0:
        torch.save(model.state_dict(), './14_output_max/select_best.pth')
        val_loss_0 = val_loss
